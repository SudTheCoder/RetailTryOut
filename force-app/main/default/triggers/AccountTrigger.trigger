...
trigger AccountTrigger on Account (after update, before delete) {
    ...
    if((ASDAC_TriggerSetting__mdt.getInstance('Enable_New_Trigger_Framework').IsActive__c == true && !ASDAC_TriggerHandlerObj.enableOldTriggerFramework)
        ...
        ASDAC_TriggerFramworkService.triggerObj = ASDAC_GlobalConstants.OBJACCOUNT;
        new ASDAC_AccountTriggerHandler().run();
    }else{
        ...
        trigObj.triggerObject = ASDAC_GlobalConstants.OBJACCOUNT;
